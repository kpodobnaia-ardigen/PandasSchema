...
from pandas_schema.validation_df import (
    CustomSeriesValidation, _BaseValidation,
...
class ValidationTestBase(unittest.TestCase):
    ...
    def validate_and_compare(self, df: list, expected_result: bool, msg: str = None, dtype: object = None):
        """
        Checks that every element in the provided series is equal to `expected_result` after validation
        :param dtype: Explicity specifies the dtype for the generated Series
        :param df: The series to check
...
class CustomSeries(ValidationTestBase):
    ...
    def setUp(self):
        self.validator = CustomSeriesValidation(lambda series: ~series.str.contains('fail'),
...
class Dtype(ValidationTestBase):
    ...
    def test_schema(self):
        ...
        schema = Schema([
            Column('wrong_dtype1', [IsDtypeValidation(np.dtype('int64'))]),
            Column('wrong_dtype2', [IsDtypeValidation(np.dtype('float64'))]),
            Column('wrong_dtype3', [IsDtypeValidation(np.dtype('int64'))]),
        ])

        errors = schema.validate(df)

        self.assertEqual(
            sorted([x.message for x in errors]),
            sorted([
                'contains dtype which is not a subclass of the required type "int64"',
                'contains dtype which is not a subclass of the required type "float64"',
                'contains dtype which is not a subclass of the required type "int64"',
            ])
        )
...
class CustomMessage(ValidationTestBase):
    ...
    def test_default_message(self):
        ...
        for error in validator.get_errors(df):
            self.assertNotEqual(error.message, self.message,
    ...
    def test_custom_message(self):
        ...
        df = pd.DataFrame([1, 2, 3])
        for error in validator.get_errors(df):
            self.assertEqual(error.message, self.message,
...
class GetErrorTests(ValidationTestBase):
    ...
    def setUp(self):
        self.values = [1.0, None, 3]

    def test_in_range_allow_empty_with_error(self):
        validator = InRangeValidation(min=4)
        errors = validator.get_errors(pd.DataFrame(self.values), allow_empty=True)
        self.assertEqual(len(errors), sum(v is not None for v in self.values))
    ...
    def test_in_range_allow_empty_false_with_error(self):
        validator = InRangeValidation(min=4)
        errors = validator.get_errors(pd.DataFrame(self.values), allow_empty=False)
        self.assertEqual(len(errors), len(self.values))
...
class PandasDtypeTests(ValidationTestBase):
    ...
    def test_valid_elements(self):
        df = pd.DataFrame(['a', 'b', 'c', None, 'A', 'B', 'C'], dtype='category')
        errors = self.validator.get_errors(df, allow_empty=True)
        self.assertEqual(len(errors), 0)

    def test_invalid_empty_elements(self):
        df = pd.DataFrame(['aa', 'bb', 'd', None], dtype='category')
        errors = self.validator.get_errors(df, allow_empty=False)
        self.assertEqual(len(errors), 4)

    def test_invalid_and_empty_elements(self):
        df = pd.DataFrame(['a', None], dtype='category')
        errors = self.validator.get_errors(df, allow_empty=False)
        self.assertEqual(len(errors), 1)

    def test_invalid_elements(self):
        df = pd.DataFrame(['aa', 'bb', 'd'], dtype='category')
        errors = self.validator.get_errors(df, allow_empty=True)
